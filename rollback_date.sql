...
-- Лог-таблицы (на случай отдельного запуска)
...
-- 2. Логи первой таблицы за указанный интервал времени
...
-- 3. Пары old/new для parent
parent_pairs AS (
    SELECT DISTINCT old_uuid, new_uuid
    FROM rollback_parent_logs
),

-- 4. Пары для child по тем же uuid и тому же интервалу времени
child_pairs AS (
